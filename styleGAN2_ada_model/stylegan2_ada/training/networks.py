...
class MappingNetwork(torch.nn.Module):
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, skip_w_avg_update=False,input_latent_space_type='z'):
        # Embed, normalize, and concat inputs.
        if input_latent_space_type=='z':
            x = None
            ...
            return x, W_latent
        elif input_latent_space_type=='w':
            x=z
            # Update moving average of W.
            if self.w_avg_beta is not None and self.training and not skip_w_avg_update:
                with torch.autograd.profiler.record_function('update_w_avg'):
...
class SynthesisBlock(torch.nn.Module):
    ...
    def forward(self, x, img, ws, force_fp32=False, fused_modconv=None,input_styleSpace_latent=False, **layer_kwargs):
        if not input_styleSpace_latent:
            ...
            # Main layers.
            if self.in_channels == 0:
                x,styles = self.conv1(x, next(w_iter), fused_modconv=fused_modconv,return_styles=True, **layer_kwargs)
                Conv_latent = torch.clone(styles)
            elif self.architecture == 'resnet':
                y = self.skip(x, gain=np.sqrt(0.5))
                x,styles = self.conv0(x, next(w_iter), fused_modconv=fused_modconv,return_styles=True, **layer_kwargs)
                Conv0_up_latent = torch.clone(styles)
                x,styles = self.conv1(x, next(w_iter), fused_modconv=fused_modconv,return_styles=True, gain=np.sqrt(0.5), **layer_kwargs)
                x = y.add_(x)
                Conv1_latent = torch.clone(styles)
            else:
                x,styles = self.conv0(x, next(w_iter), fused_modconv=fused_modconv,return_styles=True, **layer_kwargs)
                Conv0_up_latent = torch.clone(styles)
                x,styles = self.conv1(x, next(w_iter), fused_modconv=fused_modconv,return_styles=True, **layer_kwargs)
            ...
            if self.is_last or self.architecture == 'skip':
                y,styles = self.torgb(x, next(w_iter), fused_modconv=fused_modconv,return_styles=True)
                ToRGB_latent = torch.clone(styles)
...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws,input_style_latent=False, **block_kwargs):
        if not input_style_latent:
            block_ws = []
            StyleSpace_latent=[]
            ...
            for res, cur_ws in zip(self.block_resolutions, block_ws):
                ...
                if block.in_channels==0:
                    x, img,Conv_latent,ToRGB_latent = block(x, img, cur_ws, **block_kwargs)
                    StyleSpace_latent.append(Conv_latent)
                    StyleSpace_latent.append(ToRGB_latent)
                else:
                    x, img, Conv0_up_latent,Conv1_latent,ToRGB_latent = block(x, img, cur_ws, **block_kwargs)
                    StyleSpace_latent.append(Conv0_up_latent)
                    StyleSpace_latent.append(Conv1_latent)
                    StyleSpace_latent.append(ToRGB_latent)
                    if count == 7:
                        F_code = ToRGB_latent
                count +=1
            ...
            return img,StyleSpace_latent,F_code
        else:
            x = img = None
...
class Generator(torch.nn.Module):
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, input_latent_space_type='z',**synthesis_kwargs):
        ...
        if input_latent_space_type=='z':
            ws, w = self.mapping(
                ...
                input_latent_space_type=input_latent_space_type
            )
            img, styleSpace_latent,F_code = self.synthesis(ws, **synthesis_kwargs)

            FS = [F_code,ws[:,8:,:]]
            return img,w,ws,styleSpace_latent
        elif input_latent_space_type=='w':
            ws = self.mapping(
                z,
                c,
            ...
            img, styleSpace_latent,F_code = self.synthesis(ws, **synthesis_kwargs)
            return img,ws,styleSpace_latent
        elif input_latent_space_type == 'wp':

            img, styleSpace_latent,F_code = self.synthesis(z, **synthesis_kwargs)
            return img, styleSpace_latent

        elif input_latent_space_type == 's':
            img = self.synthesis(z,input_style_latent=True, **synthesis_kwargs)
            return img
